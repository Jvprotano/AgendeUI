...
export class CompanyService extends BaseService {
    ...
    getAll(page: number = 1, pageSize: number = 20): Observable<PaginatedResult<Company>> {
        return this.get(`company/by-user?page=${page}&pageSize=${pageSize}`, true);
    }
    ...
    addService(companyId: string, service: ServiceOffered): Observable<ServiceOffered> {
        return this.post(`company/${companyId}/services`, service);
    }

    updateService(companyId: string, serviceId: string, service: ServiceOffered): Observable<void> {
